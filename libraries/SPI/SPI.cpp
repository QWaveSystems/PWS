...
void SPIClass::begin(uint8_t CS) {
	if(CS != 255) {
		pinMode(CS, OUTPUT);
...
void SPIClass::endTransaction() {
	pinMode(SCK, INPUT); // QwaveSys: help on SCL/SCK using the same pin
}

void SPIClass::setDataMode(uint8_t CS,uint8_t dataMode) {
...
void SPIClass::setDataMode(uint8_t dataMode) {
    ...
    setDataMode(255, dataMode);

}

void SPIClass::setBitOrder(uint8_t bitOrder) {
    setBitOrder(255, bitOrder);
}

void SPIClass::setBitOrder(uint8_t CS,uint8_t bitOrder) {
	if(CS != 255) digitalWrite(CS, HIGH);
...
void SPIClass::setClockDivider(uint32_t clockDiv) {
    setClockDivider(255, clockDiv);
}

void SPIClass::setClockDivider(uint8_t CS,uint32_t clockDiv) {
	if(CS != 255) digitalWrite(CS, HIGH);
...
uint8_t SPIClass::transfer(uint8_t data) {
    return transfer(255, data, 0);
}

uint8_t SPIClass::transfer(uint8_t CS,uint8_t data,uint8_t end) {
	pinMode(SCK, SPECIAL); // QwaveSys: help on SCL/SCK using the same pin
	...
	if(end != 0 && CS != 255) digitalWrite(CS, HIGH);
